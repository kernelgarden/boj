...
func (parent *Node) AddChild(child *Node, weight int) {
	/*
	if _, has := child.children[parent.val]; !has {
		parent.children[child.val] = child
		parent.weights[child.val] = weight
	}
	*/
}
...
func traverse(root *Node, longestPath *int) int {
	...
	for k, v := range root.children {
		lengths = append(lengths, traverse(v, longestPath) + root.weights[k])
	}
	*/
